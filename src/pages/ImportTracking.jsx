...
function ProformaModal({ lot, busy, onClose, onMarkDone }) {
  ...
  const view = useMemo(() => {
    ...
    // รวมเป็น "No." เดียวกัน ถ้าชื่ออังกฤษเหมือนกัน (Foot Care Sticker ทุกทรง = No. เดียวกัน) —
    // ชื่อไทยต่างกันไม่เป็นไร ช่อง Product Name ไม่ merge รวม จะโชว์ชื่อไทยของแต่ละแถวเอง (ดู proformaXlsx.js)
    const groups = []
    for (const it of enriched) {
      const last = groups[groups.length - 1]
      if (last && it.name_en && it.name_en === last.name_en) last.rows.push(it)
