...
## หน้าเว็บทดลองแยก — 24 กันยายน 2026

เปิดด้วย `node scripts/orders-trial-server.mjs` จากโฟลเดอร์โปรเจกต์ แล้วเข้า http://127.0.0.1:4179 ใช้ Node 24 ไม่ต้องโหลด .env และไม่ต้องเข้า Turso

หน้าเว็บอยู่ scripts/orders-trial-ui และเซิร์ฟเวอร์อยู่ scripts/orders-trial-server.mjs ใช้ฐานใหม่ `tmp/orders-sqlite/ui-trial-v1.sqlite` แยกจากฐาน v3 และฐาน parity test ทั้งหมด ไม่แก้ src, api, package.json หรือ vite.config.js ของระบบปัจจุบัน

- เปิดเฉพาะ 127.0.0.1 ตรวจ Host/Origin และ token ของเซิร์ฟเวอร์ทดลอง; owner เป็น local-mo สำหรับผู้ใช้งานเครื่องนี้ ไม่ใช่ระบบบัญชีพร้อมเปิดออนไลน์
- รับ XLSX/CSV และ JSON snapshot ผ่านไฟล์ที่ผู้ใช้เลือก เลือกแผ่นงานได้ ไม่อ่านแฟ้มอื่นหรือข้อมูลจากชีทอัตโนมัติ
- ตรวจแพลตฟอร์ม/ธุรกิจเดียวต่อล็อต แสดงตัวอย่าง จำนวน ยอด สถานะที่ไม่นับ และ SKU กลางที่ยังไม่มี ก่อนกดนำเข้า
- ส่งเข้า upload session ชุดละ 500 แถว และ finalize หลังรับครบ มีประวัติล็อตและส่งต่อได้เมื่อเลือกไฟล์เดิมใหม่
- หน้าเว็บมีข้อมูลสมมติ 2 แถวสำหรับลอง ระบบบันทึกเฉพาะเมื่อกดนำเข้าฐานทดลอง
- หน้านี้ไม่เปิดเส้นทาง /api/import-orders ของ production และไม่มีปุ่ม push/deploy/สลับฐานจริง

ผลทดสอบ: 24 tests ผ่าน รวม XLSX round trip ของสามแพลตฟอร์มจาก fixtures, CSV ภาษาไทย, HTTP ทั้งเส้นทาง, token/Host/Origin, และชุดเดิมที่กันรายการทับกัน ทดสอบผ่านเบราว์เซอร์จริงด้วยปุ่มข้อมูลสมมติและเลือกไฟล์ CSV → ตรวจตัวอย่าง → นำเข้า → กดซ้ำ ยอดไม่เพิ่ม หน้าทดลองตอนส่งมอบมี 3 แถวสมมติ ยอด 449.50 บาท / 4 ชิ้น ไม่ใช่ยอดธุรกิจ

ข้อจำกัด: parser ทดสอบรูปแบบคอลัมน์ด้วย fixtures ยังไม่ได้ตรวจไฟล์ marketplace ต้นฉบับครบทุกแบบ ไม่ดึง product_aliases, ZIP mapping หรือข้อมูลลูกค้าจาก production; SKU กลางที่ไม่มีในไฟล์จะว่างและแสดงจำนวนให้เห็น ยอดทั้งออเดอร์ใช้ fallback ได้เฉพาะออเดอร์ที่มีแถวเดียว เพื่อไม่บวกยอดซ้ำในออเดอร์หลายสินค้า ยังไม่มีหน้าจอแก้ mapping, undo, สำรอง/กู้คืนหรือการล้างล็อตบนเว็บ ตัวเซิร์ฟเวอร์ยังใช้ SQLite ในเครื่อง ห้ามนำไปเปิดออนไลน์

คำสั่งตรวจ: `node --test test/ordersTrialServer.test.mjs test/ordersUploadStore.test.mjs test/orderLines.test.js`
...
- โค้ดทดลองอยู่ scripts/lib และ scripts/orders-trial-ui ฐานอยู่ tmp/orders-sqlite; production src, api, vite.config.js และ package.json ไม่ได้ import โมดูลทดลอง
