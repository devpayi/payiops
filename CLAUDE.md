...
- **`HRPeople.jsx`** ("ข้อมูลพนักงาน") — read-only viewer for employee profiles + job
  ...
  **separate spreadsheet** (`env HR_SHEET_ID`), NOT `mona-ops-db` — it holds PII (เลขบัตร
  ปชช, ทะเบียนบ้าน) and mona-ops-db is still link-public. Two tabs read via
  `getExternalSheet`: `HR_EMP_TAB` (default `employees`) + `HR_APPLICANT_TAB` (default
  `applicants`) — the form-response tabs, renamed to match. Headers are dynamic (whatever
  the form asks); rows returned newest-first. ID-card / house-registration images uploaded
  through the form stay in the form owner's Google Drive — cells hold the raw URL, the page
  renders it as a "เปิดไฟล์" link (boss opens with their own Google session; service
  account needs no Drive access). No `HR_SHEET_ID` set → page shows a setup checklist
  instead of erroring. No new `api/*.js` file (piggybacks `sheet-tools.js`, still 9/12).
  **Owner still-to-do:** create the sheet (do NOT link-share), share it to the service
  account (viewer), build the 2 forms, set the env, redeploy.
